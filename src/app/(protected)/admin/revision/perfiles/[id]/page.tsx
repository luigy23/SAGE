...
export default async function RevisionSolicitudDetallePage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = await params
  const solicitud = await getSolicitudParaAdmin(id)
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <Button asChild variant="ghost" size="sm" className="w-fit gap-1.5">
          <Link href="/admin/revision/perfiles">
        ...
        <CardHeader className="flex flex-row items-start justify-between gap-3 space-y-0">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Solicitud creada el{" "}
              {new Date(solicitud.createdAt).toLocaleString("es-CO")}
            </p>
            {solicitud.revisadoEn && (
              <p className="text-xs text-muted-foreground">
                Revisada el{" "}
                {new Date(solicitud.revisadoEn).toLocaleString("es-CO")}
              </p>
            )}
          </div>
